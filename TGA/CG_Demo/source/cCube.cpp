...
void Cube::draw( void )
{
	...
	//4 top
	//5 bottom
	glBindTexture(GL_TEXTURE_2D, targas[0].texID);//front
	glBegin(GL_QUADS);
	...
	glBindTexture(GL_TEXTURE_2D, targas[1].texID);//back
	glBegin(GL_QUADS);
	...
	glBindTexture(GL_TEXTURE_2D, targas[2].texID);//left
	glBegin(GL_QUADS);
	...
	glBindTexture(GL_TEXTURE_2D, targas[3].texID);//right
	glBegin(GL_QUADS);
	...
	glBindTexture(GL_TEXTURE_2D, targas[4].texID);//top
	glBegin(GL_QUADS);
	...
	glBindTexture(GL_TEXTURE_2D, targas[5].texID);//bottom
	glBegin(GL_QUADS);
...
//Mipmaps changes depending on the distance, thanks to that, we can see a blurry texture while it moves
//and the other one mantains the same texture over distance, is important to note that Mipmaps uses more Hardware Resources
